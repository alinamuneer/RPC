...
def interpolate_path(path_in_free_space,speed_factor):
    begin = False
    end = False
    interpolated_path = []
    for point in path_in_free_space:
...
def PRM_Path_Planner():
    ...
    # Perform PRM Collision checks # FIXME: This must be changed
    ...
    try:
        ...
        point_request = PointCollisionRequest() # Create Point Collision request
        # Fill data fields of message
        rospy.sleep(1) # FIXME: remove this
        point_request.position = path_start.position
        point_response = point_collision_proxy(point_request) # Send request and store response
        if point_response.free:
            free_points.append(path_start.position)
        rospy.sleep(1) # FIXME: remove this
        point_request.position = path_goal.position
        point_response = point_collision_proxy(point_request) # Send request and store response
        if point_response.free:
            free_points.append(path_goal.position)
        rospy.sleep(1) # FIXME: remove this
        ...
        # Fill data fields of message
        rospy.sleep(1) # FIXME: remove this
        line_request.start = path_start.position
        line_request.goal = path_goal.position
        ...
        rospy.sleep(1) # FIXME: remove this
        line_request.goal = point.position
        line_response = line_collision_proxy(line_request) # Send request and store response
        if line_response.free:
            free_lines.append(line_request)
        rospy.sleep(1) # FIXME: remove this
        line_request.start = path_goal.position
        line_response = line_collision_proxy(line_request) # Send request and store response
        if line_response.free:
            free_lines.append(line_request)
    ...
    # Main Loop
    interpolated_path = interpolate_path(shortest_path,150) # Interpolate path for smooth trajectory
